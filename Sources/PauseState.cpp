...
namespace Sonar
{
	...
	void PauseState::Init()
	{
		...
		this->_data->assets.GetTexture("Resume Button").setSmooth(true);
		this->_resumeButton.setTexture(this->_data->assets.GetTexture("Resume Button"));

		this->_resumeButton.setScale(0.5f, 0.5f);
		this->_resumeButton.setPosition((this->_data->window.getSize().x / 2) - (this->_resumeButton.getLocalBounds().width / 2), (this->_data->window.getSize().y / 2) - (this->_resumeButton.getLocalBounds().height / 2));
	}
